function grayoccurrance = grayoccurrance(image)
...
grayoccurrance = correlogram(rgb, map, distances);
grayoccurrance = reshape(grayoccurrance, [4 4 4]);

% consturct final correlogram using distances
grayoccurrance(:, :, 1) = grayoccurrance(:, :, 1)*distances(1);
grayoccurrance(:, :, 2) = grayoccurrance(:, :, 2)*distances(2);
grayoccurrance(:, :, 3) = grayoccurrance(:, :, 3)*distances(3);
grayoccurrance(:, :, 4) = grayoccurrance(:, :, 4)*distances(4);

% reform it to vector format
grayoccurrance = reshape(grayoccurrance, 1, 64);
