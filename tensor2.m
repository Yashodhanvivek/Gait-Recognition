function varargout = tensor2(varargin)
% TENSOR2 MATLAB code for tensor2.fig
%      TENSOR2, by itself, creates a new TENSOR2 or raises the existing
%      singleton*.
%
%      H = TENSOR2 returns the handle to a new TENSOR2 or the handle to
%      the existing singleton*.
%
%      TENSOR2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TENSOR2.M with the given input arguments.
%
%      TENSOR2('Property','Value',...) creates a new TENSOR2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before tensor2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to tensor2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help tensor2

% Last Modified by GUIDE v2.5 06-Jan-2016 15:44:38

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @tensor2_OpeningFcn, ...
                   'gui_OutputFcn',  @tensor2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before tensor2 is made visible.
function tensor2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to tensor2 (see VARARGIN)

% Choose default command line output for tensor2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes tensor2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = tensor2_OutputFcn(hObject, eventdata, handles) 
...
img=image;
  ...
  ax=imresize(img,[255 255]);
 ax=double(ax)/255;
comat=ax.*ax';
[v d]=eig(comat);
egn=((mean(v)));
egn=imresize(egn,[1 256]);

        set = [meanAmplitude msEnergy wavelet_moments egn ];
...
egn=imresize(egn,[1 256]);
       ...
       queryImageFeature = [meanAmplitude msEnergy wavelet_moments egn  str2num(name)];
